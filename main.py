...
from PySide2.QtCore import QStringListModel,Qt,QCoreApplication
...
def getNames(outError:bool=True):
    ...
    try:
        with open('./settings/settings.json',encoding='utf-8') as f:
            ...
            if len(choose) == 0 and outError:#没指定
                QMessageBox.critical(main.ui,'警告','没有设置点名的班级和范围。请点确定后从设置里面调一下。')
                classNames=[];names=[]
    except (FileNotFoundError,KeyError):#没指定
        if outError:
            QMessageBox.critical(main.ui,'警告','没有设置点名的班级和范围。请点确定后从设置里面调一下。')
        classNames=[];names=[]
    except JSONDecodeError:
...
QMenu::item/*表头样式*/
{
    ...
    def next(self,showResult: bool=True) -> str:
        ...
        if len(names) == 0:#没有设置范围
            QMessageBox.critical(self.ui,'警告','没有设置点名的班级和范围。待会从设置里面调一下。')
            self.changeClass()
            return
        name=choice(names)
        if isLog:
    ...
    def many(self,n:int) -> None:
        """连点，多次调用next()实现"""
        if n == 0:
            n,ok=QInputDialog.getInt(self.ui,'输入','请输入连点个数',value=5,minValue=1,maxValue=100)
            if not ok:return
        name=[]
        logger.success('开始连续点{}个名字！',n)
        for i in range(n):
            name.append(self.next(False))
        logger.success('连续点{}个名字完毕！',n)
        result=QMessageBox.question(self.ui,'连点结果,保存吗？','\n'.join(name))#保存
        if result == QMessageBox.StandardButton.Yes:
            filename,ok=QFileDialog.getSaveFileName(self.ui,'保存点名结果',filter='文本文件 (*.txt)')
            if filename == '':return#中途取消
...
class EditWindow:
    ...
    def addClass(self) -> None:
        """添加新的班级"""
        newClassName,okPressed=QInputDialog.getText(self.ui,'新建班级','输入新的班级的名字：')
        if not okPressed:
            return#没给名字
        self.all_ClassNames.append(newClassName)
        self.slm.setStringList(self.all_ClassNames)#给slm加一项
        self.all_name[newClassName] = []
        self.ui.listView.setCurrentIndex(self.slm.index(len(self.all_ClassNames),0))
        self.ui.PromptLabel.setText('请输入或粘贴名单。')
    def delClass(self) -> None:
        """删除当前班级"""
        be_deleted=self.ui.listView.selectionModel().selectedIndexes()[0].data()#获取当前选择的班级名字
    ...
    def saveResult(self) -> None:
        """保存结果"""
        self.edit_class_name=self.ui.listView.selectionModel().selectedIndexes()[0].data()#班级名称
        with open('./settings/name_sheet.json',encoding='utf-8') as f:
            old_set:dict=load(f)
            old_set[self.edit_class_name] = self.ui.NamesText.toPlainText().strip().split('\n')
        with open('./settings/name_sheet.json',encoding='utf-8',mode='w') as f:
            dump(old_set,f, ensure_ascii=False)
        #刷新姓名列表，目的是去掉当前名单中可能出现的这些人
        getNames(outError=False)
    ...
    def textChanged(self) -> None:
        ...
        if self.ui.NamesText.toPlainText().strip() == '':
            ...
            self.ui.PromptLabel.setText('正在编辑班级：{}，有{}个人'.format(self.edit_class_name,self.ui.NamesText.toPlainText().strip().count("\n")+1))
main=MainWindow()
getNames(outError=False)
getIsLog()
if isLog:
